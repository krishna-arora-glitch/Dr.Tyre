...
export function lockSetup() {
  const inputs = [
    document.getElementById('setup-ruleset'), 
    document.getElementById('setup-tyre'), 
    document.getElementById('setup-fuel'), 
    document.getElementById('setup-aero-downforce'), 
    document.getElementById('setup-aero-balance'), 
    document.getElementById('setup-mech-balance'), 
    document.getElementById('setup-brake-bias'), 
    document.getElementById('setup-energy')
  ];
  inputs.forEach(input => {
  ...
  if (btn) {
    btn.textContent = 'PARC FERMÉ - LOCKED';
    btn.disabled = true;
  }
  isSetupLocked = true;
}
...
export function unlockSetup() {
  ...
  isSetupLocked = false;
  updateSetupState(); // Re-evaluates legality and re-enables button if valid
}
...
function updateSetupState() {
  ...
  if (isSetupLocked) {
    // Force override if Parc Fermé is active
    DOM.btnConfirm.disabled = true;
    DOM.btnConfirm.textContent = 'PARC FERMÉ - LOCKED';
    DOM.legalityPanel.style.borderTopColor = 'var(--cyan)';
    DOM.legalityTitle.textContent = '🔒 PARC FERMÉ ACTIVE';
    DOM.legalityTitle.style.color = 'var(--cyan)';
  } else if (isLegal) {
