...
import { activePrior, firePriorChange } from '../fuel-prior-state.js';

// ── Helpers ────────────────────────────────────────────────────
let sundayOracleChartInstance = null;
...
export function updateResearchWithSimulationState(simState, modelData) {
  ...
  // Parc Fermé: Lock the fuel slider if race is active or finished
  const slider = document.getElementById('fuel-prior-slider');
  const panel = document.getElementById('fuel-prior-panel');
  if (slider && panel) {
    const isLocked = simState.active || simState.lap > 1;
    if (isLocked && !slider.disabled) {
      ...
      if (titleEl && !titleEl.textContent.includes('PARC FERMÉ')) {
        titleEl.innerHTML = 'FUEL ASSUMPTION <span style="color:var(--red);margin-left:8px;font-size:0.7rem;">[PARC FERMÉ LOCKED]</span>';
      }
    } else if (!isLocked && slider.disabled) {
...
export function initTyreIntelligence(data) {
  ...
  order.forEach(comp => {
    ...
    const degPerLap = c.deg_per_lap_linear !== undefined ? c.deg_per_lap_linear : (c.deg_linear || 0);
...
export function initGhostBaseline(data) {
  ...
  renderGhostBaselineChart('chart-ghost-baseline', data);
    ...
    mathPanel.querySelector('#math-content').innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;font-family:var(--font-mono);font-size:0.75rem;">
        ...
        <div>
          ...
          <div>${activePrior} s/kg ${dataLabel('ASSUMED')}</div>
        </div>
        <div>
      ...
      <div style="margin-top:16px;padding:12px;background:rgba(0,0,0,0.02);border-radius:6px;">
  ...
  // ── Fuel-Prior Sensitivity Slider Wiring ──
  initFuelPriorSlider();
}

let sliderWired = false;

function initFuelPriorSlider() {
  const slider = document.getElementById('fuel-prior-slider');
  const valueDisplay = document.getElementById('fuel-prior-value');
  const tickContainer = document.querySelector('.fuel-prior-ticks');

  if (!slider || !valueDisplay) return;

  // Sync slider to current activePrior
  slider.value = activePrior;
  valueDisplay.textContent = activePrior;

  // Update tick highlights
  updateTickHighlights(tickContainer, activePrior);

  // Only wire event listeners once
  if (sliderWired) return;
  sliderWired = true;

  // Slider input: instant visual feedback + data re-render
  slider.addEventListener('input', (e) => {
    const val = parseFloat(e.target.value).toFixed(2);
    valueDisplay.textContent = val;
    updateTickHighlights(tickContainer, val);
    firePriorChange(val);
  });

  // Make tick marks clickable
  if (tickContainer) {
    tickContainer.querySelectorAll('span').forEach(tick => {
...
function updateTickHighlights(container, activeVal) {
  if (!container) return;
  container.querySelectorAll('span').forEach(tick => {
    if (tick.dataset.val === activeVal) {
...
export function initCompare(data) {
  ...
  const degA = compA.deg_per_lap_linear !== undefined ? compA.deg_per_lap_linear : (compA.deg_linear || 0);
  const degB = compB.deg_per_lap_linear !== undefined ? compB.deg_per_lap_linear : (compB.deg_linear || 0);

  const diff = (degA - degB).toFixed(3);
  ...
  container.innerHTML = `
    <div class="compare-grid">
      <div class="compare-card" style="border-top:3px solid ${compoundColors[nameA]};">
        ...
        ${metricCard('Est. Pace Loss', `+${degA.toFixed(3)}`, 's/lap', '', dataLabel('ESTIMATED'))}
      ...
      <div class="compare-card" style="border-top:3px solid ${compoundColors[nameB]};">
        ...
        ${metricCard('Est. Pace Loss', `+${degB.toFixed(3)}`, 's/lap', '', dataLabel('ESTIMATED'))}
    ...
    <div class="compare-insight">
      <h4>MODEL INSIGHT</h4>
      <p><strong>${nameA}</strong> shows an estimated tyre-induced pace-loss rate of <strong>+${degA.toFixed(3)} s/lap</strong> 
      compared to <strong>${nameB}</strong> at <strong>+${degB.toFixed(3)} s/lap</strong>. 
      ...
      if (q === '1') {
        ...
        answerDiv.innerHTML = `
          <div class="investigate-answer">
            ...
            <div style="text-align:center;margin-top:24px;padding:16px;background:rgba(0,0,0,0.03);border-radius:8px;">
...
export async function initValidation(data) {
  const container = document.getElementById('validation-content');
  if (!container) return;

  if (!data?.charts?.validation?.metrics) {
    container.innerHTML = '<p style="color: var(--amber);">No validation metrics available for this session.</p>';
    return;
  }
  ...
  try {
    const res = await fetch('/validation_output.json');
    if (res.ok) {
      validationData = await res.json();
    }
  } catch (err) {
    console.warn("Could not load validation_output.json", err);
  }
  ...
  container.innerHTML = `
    ...
    <div class="glass-panel" style="margin-top:24px; border-left:4px solid var(--amber);">
      ...
      <div style="height:350px;">
        <canvas id="chart-sunday-oracle"></canvas>
      </div>
      
      <div id="sunday-metrics" class="metric-grid" style="margin-top:16px;"></div>
    </div>
    ` : ''}
  `;
  ...
  if (validationData) {
    ...
    // Render Chart
    const ctx = document.getElementById('chart-sunday-oracle');
    if (sundayOracleChartInstance) {
      sundayOracleChartInstance.destroy();
    }
    sundayOracleChartInstance = new Chart(ctx, {
      type: 'line',
      data: {
        labels: sundayAges,
        datasets: [
          ...
          },
          {
            ...
            data: sundayAges.map((x, i) => ({ x, y: sundayPaceLoss[i] })),
            borderColor: '#000000',
      ...
      options: {
        ...
        scales: {
          ...
          y: {
            border: { display: true, color: '#000000', width: 2 },
            title: { display: true, text: 'Pace Loss (Seconds)', color: '#000000' },
            grid: { tickColor: '#000000', color: 'rgba(0,0,0,0.06)' },
            ticks: { color: '#000000' }
          }
        }
...
export function initProvenance(data) {
  ...
  container.innerHTML = `
    <div class="provenance-grid">
      ...
      <div class="provenance-card"><div class="provenance-card-label">EVENT</div><div class="provenance-card-value">${CIRCUITS[raceConfig.trackId]?.name || data.race_info?.name || 'N/A'}</div></div>
  ...
  container.innerHTML = `
    <div class="glass-panel" style="max-width:700px;margin:0 auto;">
      <div class="report-section">
        <h3>DATA SOURCE</h3>
        <div class="report-row"><span class="report-row-label">Session</span><span class="report-row-value">${CIRCUITS[raceConfig.trackId]?.name.toUpperCase() || data.race_info?.name || 'N/A'} FP2</span></div>
      ...
      <div class="report-section">
        <h3>DIAGNOSTICS</h3>
        <div class="report-row"><span class="report-row-label">Telemetry</span><span class="report-row-value">${data.telemetry?.enabled ? 'ACTIVE' : 'INACTIVE'}</span></div>
        <div class="report-row"><span class="report-row-label">Traffic Detector</span><span class="report-row-value">SPEED PROFILE</span></div>
        <div class="report-row"><span class="report-row-label">Lap Stress</span><span class="report-row-value">${data.stress?.enabled ? 'ENABLED' : 'DISABLED'}</span></div>
        <div class="report-row"><span class="report-row-label">Exp. Speed Confidence</span><span class="report-row-value">${data.telemetry?.baseline_confidence || 'HIGH'}</span></div>
      </div>
      ...
      <div class="report-section">
        ...
        <div class="report-row"><span class="report-row-label">Traffic Filtered</span><span class="report-row-value">${data.traffic?.statuses?.traffic || 0} laps</span></div>
      </div>
      ...
      <div class="report-section">
        ...
        <div class="report-row"><span class="report-row-label">MAE</span><span class="report-row-value">${(val.mae || 0).toFixed(3)}s</span></div>
        <div class="report-row"><span class="report-row-label">RMSE</span><span class="report-row-value">${(val.rmse || 0).toFixed(3)}s</span></div>
        <div class="report-row"><span class="report-row-label">Mean Bias</span><span class="report-row-value">${(val.mean_bias || 0) > 0 ? '+' : ''}${(val.mean_bias || 0).toFixed(3)}s</span></div>
      </div>` : ''}
...
function initMethodologyPipeline() {
  ...
  const descriptions = [
    { title: 'OBSERVE', text: `FastF1 lap timing, tyre compound, stint boundaries, and session data are loaded from the ${CIRCUITS[raceConfig.trackId]?.name || 'Singapore'} GP FP2 practice session.` },
